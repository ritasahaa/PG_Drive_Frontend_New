// AdminWebhookLogs.jsx
// Industry-level admin webhook logs page
import React, { useEffect, useState } from 'react';
import axios from 'axios';

export default function AdminWebhookLogs() {
  const [logs, setLogs] = useState([]);
  const [filters, setFilters] = useState({ event: '', status: '', user: '', dateFrom: '', dateTo: '' });
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const res = await axios.get(`/api/adminWebhookLogs?${params}`);
  ...
  const handleDelete = async id => {
    ...
    setSuccess('');
    try {
      await axios.delete(`/api/adminWebhookLogs/${id}`);
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 bg-white rounded shadow">
      <h2 className="text-xl font-bold mb-4">Admin Webhook Logs</h2>
      <form onSubmit={handleFilterSubmit} className="mb-6 flex gap-4 flex-wrap items-end">
        <input type="text" name="event" value={filters.event} onChange={handleFilterChange} placeholder="Event" className="p-2 border rounded" />
