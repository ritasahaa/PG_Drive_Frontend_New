// AdminAuditLogs.jsx
// Admin audit logs page (industry-level)
...
export default function AdminAuditLogs() {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const res = await axios.get(`/api/adminAuditLogs?${params}`);
  ...
  const handleDelete = async id => {
    ...
    setSuccess('');
    try {
      await axios.delete(`/api/adminAuditLogs/${id}`);
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 bg-white rounded shadow">
      <h2 className="text-xl font-bold mb-4">Admin Audit Logs</h2>
      <form onSubmit={handleFilterSubmit} className="mb-6 flex gap-4 flex-wrap items-end">
        ...
        <input type="text" name="action" value={filters.action} onChange={handleFilterChange} placeholder="Action" className="p-2 border rounded" />
