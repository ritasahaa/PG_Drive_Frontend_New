// AdminGDPRConsent.jsx
// Industry-level admin GDPR/data consent page
import React, { useEffect, useState } from 'react';
import axios from 'axios';

export default function AdminGDPRConsent() {
  const [consents, setConsents] = useState([]);
  const [filters, setFilters] = useState({ user: '', status: '', dateFrom: '', dateTo: '' });
  ...
  const fetchConsents = async () => {
    ...
    try {
      ...
      const res = await axios.get(`/api/adminGDPR?${params}`);
  ...
  const handleStatusUpdate = async (id, status) => {
    ...
    setSuccess('');
    try {
      await axios.patch(`/api/adminGDPR/${id}/status`, { status });
      setSuccess(`Consent ${status}`);
      fetchConsents();
    } catch (err) {
  ...
  const handleDelete = async id => {
    ...
    setSuccess('');
    try {
      await axios.delete(`/api/adminGDPR/${id}`);
      setSuccess('Consent record deleted');
      fetchConsents();
    } catch (err) {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 bg-white rounded shadow">
      <h2 className="text-xl font-bold mb-4">Admin GDPR/Data Consent</h2>
      ...
      {loading ? <div>Loading...</div> : (
        <table className="w-full border">
          ...
          <tbody>
            {consents.map(c => (
              <tr key={c._id}>
                <td className="border px-2 py-1">{c.user ? c.user.name : '-'}</td>
                <td className="border px-2 py-1">{c.status}</td>
                <td className="border px-2 py-1">{JSON.stringify(c.details)}</td>
                <td className="border px-2 py-1">{new Date(c.createdAt).toLocaleString()}</td>
                <td className="border px-2 py-1 flex gap-2">
                  <button className="bg-green-600 text-white px-2 py-1 rounded" onClick={() => handleStatusUpdate(c._id, 'approved')}>
                    Approve
                  </button>
                  <button className="bg-yellow-600 text-white px-2 py-1 rounded" onClick={() => handleStatusUpdate(c._id, 'rejected')}>
                    Reject
                  </button>
                  <button className="bg-red-600 text-white px-2 py-1 rounded" onClick={() => handleDelete(c._id)}>
                    Delete
                  </button>
                </td>
              </tr>
